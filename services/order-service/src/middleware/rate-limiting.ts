...
class RateLimitingMiddleware {
  ...
  constructor() {
    ...
    const rateLimiterOptions: IRateLimiterOptions = {
      ...
      points: config.RATE_LIMIT_MAX_REQUESTS, // Maximum requests
      duration: Math.ceil(config.RATE_LIMIT_WINDOW_MS / 1000), // Window in seconds
      blockDuration: Math.ceil(config.RATE_LIMIT_WINDOW_MS / 1000), // Block for the same duration
