...
export type LogEventType = 'log.message.created' | 'log.message.updated' | 'log.error.created' | 'log.error.updated' | 'log.warning.created' | 'log.warning.updated' | 'log.info.created' | 'log.info.updated' | 'log.debug.created' | 'log.debug.updated';
/**
 * Logger information
 */
 ...
 */
export declare function isLogEvent(event: BaseEvent): event is LogEvent;
/**
 * Type guard to check if a log event is an error event
 */
export declare function isErrorLogEvent(event: LogEvent): boolean;
/**
 * Helper to create a log event with required fields
 */
export declare function createLogEvent(data: LogEventData, baseFields: Omit<BaseEvent, 'eventType' | 'data'>): LogEvent;
/**
 * Log level hierarchy for filtering and comparison
 */
export declare const LOG_LEVEL_HIERARCHY: Record<LogLevel, number>;
/**
 * Check if a log level meets the minimum threshold
 */
export declare function isLogLevelEnabled(level: LogLevel, minLevel: LogLevel): boolean;
/**
 * Sanitize sensitive data from log message and structured fields
 */
export declare function sanitizeLogData(data: LogEventData): LogEventData;
